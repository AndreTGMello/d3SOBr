...
treemapzoomData = d3sparql.query(endpoint, sparql, storeTreemapzoomData);
printLog(treemapzoomData);
}

function storeTreemapzoomData(json){
	var tamanho = json.results.bindings.length;
  console.log(tamanho);
...
console.log(objectFirst);
    ...
    for (var j = 0; j < treemapzoomData.length; j++) {
      if (treemapzoomData[j] == undefined) {
        existe = false;
        break;
      }
      if (treemapzoomData[j].key == key) {
        existe = true;
        break;
      }
      aux++;
    ...
    var objectSecond = {};
    objectTituloOrigem.key = valueSecond;
    for (var j = 0; j < treemapzoomData[aux].length; j++) {
      if (treemapzoomData[aux][j] == undefined) {
        existeDois = false;
        break;
      }
      else if (treemapzoomData[aux][j].key == valueTituloOrigem) {
        existeDois = true;
        break;
      }
      auxDois++;
